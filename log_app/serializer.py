...
class LogMpesaSerializer(serializers.ModelSerializer):
    class Meta:
        model = LogMpesa 
        fields = ('id','fuel','date','transaction_number','user','logged_by','customer_name','customer_phone_number','amount','amount_transferred_to_bank','daily_total','cumulative_amount','first_logged','last_edited')

class LogCreditCardSerializer(serializers.ModelSerializer):
    class Meta:
        model = LogCreditCard
        fields = ('id','fuel','date','card_name','card_number','amount','logged_by')
